  ...
  for (let i = 0; i < orderedByColor.length; i++) {
    ...
    colorElement.addEventListener("click", (e) => {
      if (localStorage.getItem(`${selectedCompany}1`)) {
        const colorElements1 = document.querySelectorAll(
          "#palette .color-element"
        );
        colorElements1.forEach((colorEl) => {
          if (colorEl.textContent === e.target.textContent) {
    ...
    colorElement2.addEventListener("click", (e) => {
      if (localStorage.getItem(`${selectedCompany}1`)) {
        const colorElements2 = document.querySelectorAll(
          "#palette-2 .color-element"
        );
        colorElements2.forEach((colorEl) => {
          if (colorEl.textContent === e.target.textContent) {
            colorEl.style.border = "3px solid #000";
            colorEl.style.borderRadius = "50%";
          } else {
